...
Selection: the six built packages are exactly ccc_release.V0317_SOURCE_BUILD_PACKAGES; the remaining
...
def _validate_six_record(raw_bytes: bytes, solution: dict, metadata: dict) -> dict:
    ...
    if not isinstance(recs, list) or len(recs) != _R.V0317_BUILT_COUNT:
        raise GenError(f"six-record must be a JSON list of exactly {_R.V0317_BUILT_COUNT} records")
    approved = set(_R.V0317_SOURCE_BUILD_PACKAGES)
...
def generate(*, metadata_path, metadata_sha, tags_path, tags_sha, six_record_path, six_record_sha,
    ...
    if len(solution) != _R.V0317_TOTAL_COUNT:
        raise GenError(f"solution lock must have exactly {_R.V0317_TOTAL_COUNT} pins; got {len(solution)}")
    ...
    if len(validated["wheels"]) != _R.V0317_REUSED_COUNT:
