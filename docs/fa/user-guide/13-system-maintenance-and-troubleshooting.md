...
ارتقاهای روزمره از به‌روزرسانی تک‌کلیکی داشبورد استفاده می‌کنند؛ اجرای دستی `update.sh` از طریق SSH برای نصب اولیه، بازیابی و نگهداری اضطراری حفظ شده است. اگر به‌روزرسانی به‌صورت خودکار بازگردانی شود، گره شما نسخهٔ قبلی را نگه می‌دارد — با استفاده از گزارش worker (`/var/lib/conduit-cc/update-worker.log`) و فایل وضعیت (`/var/lib/conduit-cc/update-status.json`) عیب‌یابی کنید و دوباره تلاش نمایید. جریان کامل: **[به‌روزرسانی نرم‌افزار و انتشارهای امضاشده](software-updates-and-signed-releases.md)**.
