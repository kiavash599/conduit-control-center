...
CCC مراحل زیر را انجام می‌دهد:

![ایجاد Backup: CCC پیکربندی را جمع‌آوری، بسته‌بندی و رمزنگاری می‌کند و فایل را دانلود می‌کند](../../diagrams/svg/backup-create-flow.svg)

*CCC پیکربندی را جمع‌آوری می‌کند، داده‌ها را بسته‌بندی و با AES-256-GCM (کلید مشتق‌شده با scrypt) رمزنگاری می‌کند و فایل Backup را برای دانلود به مرورگر شما می‌فرستد.*
...
![جریان Restore با Rollback خودکار: CCC بازرسی می‌کند، سازگاری را بررسی می‌کند، بازیابی می‌کند، سلامت را تأیید می‌کند و در صورت شکست بازمی‌گرداند](../../diagrams/svg/restore-rollback-flow.svg)

*Restore یک بازنویسی کورکورانه نیست: CCC ابتدا Backup را بازرسی می‌کند، سازگاری را بررسی می‌کند، پس از تأیید بازیابی می‌کند و سپس سلامت را بررسی می‌کند — اگر بررسی سلامت شکست بخورد، به‌طور خودکار Rollback می‌کند.*
