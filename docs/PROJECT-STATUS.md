...
| One-Click CCC Update (Feature 2) | ✅ Production-proven (v0.3.11) | Dashboard Software Updates → `/api/update` → `ccc-update-apply` → `update.sh --ccc-only`; GitHub Releases (stable); async status + reconnect + auto-rollback; no auto-update; Conduit Core out of scope. The v0.3.3 validation release exposed a `/run/lock` EROFS, fixed in v0.3.4 (lock → `/var/lib/conduit-cc/.update.lock`). **Validated end-to-end on a live Raspberry Pi (v0.3.10 → v0.3.11, 2026-07-02) on the B1 transient-unit engine: `state=success`, services active, `/opt/conduit-cc/bin` preserved, `ProtectSystem=strict` unchanged.** | `docs/closure/one-click-update-closure.md`; tags `v0.3.2`…`v0.3.11` |
