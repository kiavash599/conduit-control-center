...
**Between implementation phases.** Latest release: **v0.3.21**. The v0.3.19 → v0.3.21 line shipped the first-transition rollback-path correction (v0.3.19), the opt-in traffic recording toggle + gradient-area history chart (v0.3.20), and the content-hash static-asset cache-bust fix (v0.3.21), with the dashboard **one-click update validated on both RPi2 (armv7l) and RPi4 (aarch64)**. **BL-0001 (Signed-Release Toolkit) is closed** — after running the SRT ceremony manually three times across v0.3.19–0.3.21, the local toolkit was brought onto the current two-platform contract, a read-only post-publish verification mode was added, and `docs/runbooks/owner-release-guide.md` now documents the ordered ceremony; dogfooded against the published v0.3.21 assets (`docs/closure/bl-0001-signed-release-toolkit-closure.md`). Every irreversible public step (tag push, publish) remains a manual Owner action. **Remaining accepted backlog:** BL-0002's residual clean-image install validation on the Raspberry Pi 2. No implementation phase is open until a driver is approved via a Value Gate. Candidate drivers also live in the reconciled roadmap (`docs/roadmap/CCC_Product_Roadmap_v1.md`, *Next Phase / Candidate Drivers*). Being between phases does not mean the project is inactive: architecture, documentation, ADR, research, discovery, and planning may continue.
