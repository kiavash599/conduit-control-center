...
**Revision:** 1.27<br>
**Date:** 2026-07-25<br>
**Status:** Reconciled to v0.3.20 (released 2026-07-25)<br>
...
| 0.3.19 | `v0.3.19` | 2026-07-25 | **First-transition rollback-path correction** — `_verify_store_ownership` delegates to the symlink-aware `runtime_store.py::validate_store_shape` (Fix 1); rollback step 5f re-applies nginx via `ccc-apply-https-port` (Fix 2). First **signed** public release (Logical Tree Digest v1, manifest format 3; aarch64 + armv7l assets digest- and SSHSIG-verified). Qualified on RPi2 (armv7l) + RPi4 (aarch64). **Note:** tags v0.3.16, v0.3.17, v0.3.18 were prepared but **never published** and are superseded by v0.3.19; v0.3.17's SRT-identity incident (`docs/incidents/v0.3.17-unreleased.md`) is remediated here |
| 0.3.20 | `v0.3.20` | 2026-07-25 | **Opt-in traffic recording toggle + gradient-area history chart.** Settings switch + Lifetime & History card CTA enable/disable the collector live (persisted in the `app_settings` store — no privileged `config.json` write); the history chart is redesigned as smooth gradient-filled area curves. No dependency changes (SRT reused the v0.3.18 wheelhouse). Also the **first end-to-end validation of the dashboard one-click update path** (v0.3.19 → v0.3.20) on both RPi2 (armv7l) and RPi4 (aarch64). Follow-up: the mtime-based `static_url` cache-bust is defeated by deterministic artifacts' fixed `mtime=0` (token constant across releases → stale cached JS/CSS until a hard refresh); v0.3.21 switches it to a content-hash token |
...
| 1.26 | 2026-07-25 | CCC Development Team | **v0.3.19 released + v0.3.15–v0.3.18 drift reconciled.** v0.3.19 published — the first public release after v0.3.14: first-transition rollback-path correction (Fix 1 symlink-aware store-shape validation; Fix 2 nginx port re-apply delegation), shipped as the first **signed** release (Logical Tree Digest v1, manifest format 3), digest- and SSHSIG-verified on both aarch64 + armv7l against the `conduit-control-center-publisher` identity, and qualified on RPi2 + RPi4. Added v0.3.19 to Release History; header status flipped to *Reconciled to v0.3.19*. **Reconciled the deferred v0.3.15–v0.3.18 drift** rev 1.25 left open: those versions were tagged/RC-prepared but **never published** (no GitHub Release) and are superseded by v0.3.19; the v0.3.17 SRT-identity incident is now remediated — its Logical Tree Digest v1 fix reached the public in v0.3.19, not the unpublished v0.3.18 (`docs/incidents/v0.3.17-unreleased.md`, Status → *closed — remediation shipped in v0.3.19*). Documentation-only; additive; historical rev-≤1.25 rows intact; no feature scope changed. |
| 1.27 | 2026-07-25 | CCC Development Team | **v0.3.20 released + one-click update path validated + Raspberry Pi 2 documented.** v0.3.20 published: opt-in traffic recording toggle (Settings switch + card CTA, `app_settings`-backed with no privileged `config.json` write) and a gradient-area redesign of the Lifetime & History chart; no dependency changes (SRT reused the v0.3.18 wheelhouse, `--version 0.3.20 --git-ref v0.3.20`, four assets digest- and SSHSIG-verified). Added v0.3.20 to Release History; header status → *Reconciled to v0.3.20*. This release provided the **first end-to-end validation of the dashboard one-click update path** (v0.3.19 → v0.3.20) on both RPi2 (armv7l) and RPi4 (aarch64). Added **Raspberry Pi 2 Model B (32-bit armv7l)** to the supported hardware in the user guide (EN + FA), reflecting the armv7l release path. **Known follow-up (v0.3.21):** the `static_url` cache-bust token is the file mtime, but deterministic release artifacts ship every file with `mtime=0`, so the token never changes between releases and browsers serve stale cached JS/CSS until a hard refresh — the fix is a content-hash token (per the recorded "consume deterministic artifacts by content" lesson). Documentation-only; additive; historical rev-≤1.26 rows intact; no feature scope changed. |
