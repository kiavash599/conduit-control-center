# Manual SSH migration to v0.3.16 (legacy RPi4, arm64)
...
2. Download to the Pi: `ccc-0.3.16-aarch64.tar.gz`, `ccc-0.3.16.manifest.json`,
   `ccc-0.3.16.manifest.json.sig`, and obtain the publisher `allowed_signers` out-of-band.
...
5. Confirm `/api/health` reports 0.3.16 and services are active. If Phase 3/4 fails after downtime,
