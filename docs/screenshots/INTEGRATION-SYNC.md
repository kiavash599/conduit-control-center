...
| L16 | tls-setup.md (EN) + fa/tls-setup.md (FA) | Before You Begin (SSL/TLS mode) | cloudflare-domain-active.png (reuse), cloudflare-ssl-domain-overview.png, cloudflare-ssl-overview.png, cloudflare-ssl-full-strict.png | multi-series | done | done | done | TLS; **shared** by EN + FA TLS guides (single image set, no FA copies; Epic D Phase 1–2); `cloudflare-domain-active.png` reused from ch05 §5.10 |
| L17 | tls-setup.md (EN) + fa/tls-setup.md (FA) | A.1 Create the certificate | cloudflare-origin-server.png, cloudflare-origin-create.png, cloudflare-origin-configure.png, cloudflare-origin-cert-key.png | multi-series | done | done | done | TLS; **shared** by EN + FA TLS guides (single image set; Epic D Phase 1–2); cert+key screen fully redacted (Phase 1) |
